...
export default function AdminNewOrderPage() {
  ...
  const cityId = selectedSeller?.city_id ?? null;
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error: sellersError } = await supabase
        .from('profiles')
        .select('id, display_name, city_id, is_active, cities(name)')
        .eq('role', 'seller')
        .eq('is_active', true)
        .order('display_name');
      if (cancelled) return;
      if (sellersError) {
        if (isSessionError(sellersError)) {
          signOut(t('auth.sessionExpired'));
          return;
        }
        setError(sellersError.message);
        setSellers([]);
      } else {
        setSellers(
          (data || []).map((row) => {
            const city = row.cities as { name?: string } | { name?: string }[] | null;
            const cityRow = Array.isArray(city) ? city[0] : city;
            return {
              id: row.id,
  ...
  useEffect(() => {
    ...
    if (!cityId) {
      setProducts([]);
      setLoading(false);
      setError('У выбранного продавца не указан город. Создание заказа невозможно.');
      return;
    }
    let cancelled = false;
    setLoading(true);
  ...
  }, [authReady, profile?.id, selectedSellerId, cityId, signOut]);
  ...
  async function handleConfirm() {
    if (!canSubmit || !profile?.id || !selectedSeller?.id || !cityId) return;
    ...
    try {
      ...
      router.refresh();
      router.push(`/admin/orders${orderId ? `?id=${orderId}` : ''}`);
    } catch (err) {
      ...
      <div className="min-h-screen pb-24 md:pb-4 w-full max-w-full overflow-x-hidden bg-white">
        <div className="p-3 sm:p-4 w-full max-w-full min-w-0">
          <div className="flex items-center gap-2 mb-3 sm:mb-4">
            <Link href="/admin" className="text-blue-600 font-medium text-sm">
              {t('common.back')}
            </Link>
          </div>

          <h1 className="text-lg sm:text-2xl font-bold mb-3 sm:mb-4 text-gray-900">Новый заказ от имени продавца</h1>

          {error && <div className="mb-3 text-red-600 text-sm">{error}</div>}

          <div className="mb-4">
            <label className="block text-sm font-medium mb-1 text-gray-900">Продавец</label>
            <select
            ...
            >
              <option value="">{sellersLoading ? 'Загрузка продавцов...' : 'Выберите продавца'}</option>
              {sellers.map((seller) => (
                <option key={seller.id} value={seller.id}>
                  {seller.display_name || seller.id.slice(0, 8)}{seller.city_name ? ` · ${seller.city_name}` : ' · город не указан'}
                </option>
              ))}
            </select>
            {selectedSeller && !selectedSeller.city_id && (
              <p className="mt-2 text-sm text-red-600">У этого продавца не указан город. Создание заказа невозможно.</p>
            )}
          </div>
          ...
          <input
            ...
            value={search}
            disabled={!selectedSellerId || !cityId}
          ...
          {!selectedSellerId ? (
            <p className="text-sm text-gray-600 mb-4">Сначала выберите продавца.</p>
          ) : loading ? (
